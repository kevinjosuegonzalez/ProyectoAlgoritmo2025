#Crea la lista 'cursos' vacía para almacenar los nombres de los cursos.
cursos = []

#Crea la lista 'notas' vacía para almacenar las notas correspondientes.
notas = []

#Crea la lista 'historial_cambios' que funcionará como una pila para registrar cambios
    ...
    while True:
        ...
        if nombre == "":
            ...
            try:
                ...
                if 0 <= nota <= 100:
                    #Agrega el nombre del curso a la lista 'cursos'.
        ...
        # Pregunta al usuario si desea repetir la acción (
...
def mostrar_cursos():
    while True:
        # Verifica si la lista 'cursos' está vacía
        if not cursos:
...
# Define la función calcular_promedio
...
# Define la función contar_aprobados
def contar_aprobados():
    while True:
        if not notas:
            print("No hay cursos registrados.")
        else:
            # Cuenta cuántas notas son mayores o iguales a 60
            aprobados = sum(1 for n in notas if n >= 60)
            # Calcula la cantidad de cursos reprobados restando aprobados del total.
...
# Define la funcion buscar_lineal
def buscar_lineal():
    while True:
        if not cursos:
            print("No hay cursos registrados.")
...
# Define la funcion actualizar_nota
def actualizar_nota():
    while True:
        if not cursos:
            print("No hay cursos registrados.")
            ...
            for i in range(len(cursos)):
                if nombre == cursos[i].lower():
                    try:
                        ...
                        if 0 <= nueva_nota <= 100:
 # Registra en la pila 'historial_cambios' una cadena con la operación realizada para llevar un historial
                            historial_cambios.append( f"Se actualizó: {cursos[i]} - Nota anterior: {notas[i]} → Nueva nota: {nueva_nota}")
                            notas[i] = nueva_nota
...
# Define la función eliminar_curso
def eliminar_curso():
    while True:
        if not cursos:
            print("No hay cursos registrados.")
...
# Define la función ordenar_por_nota
        ...
        if not cursos:
            ...
            for i in range(len(notas) - 1):
                for j in range(len(notas) - i - 1):
                    # Compara dos notas adyacentes para ordenar de mayor a menor
...
# Define la función ordenar_por_nombre
    ...
    while True:
        if not cursos:
            ...
            for i in range(1, len(cursos)):
                #En inserción, guarda el nombre y la nota actuales como 'clave' para desplazar elementos y encontrar su posición correcta.
                key_nombre = cursos[i]
...
# Define la funcion buscar_binaria
def buscar_binaria():
    while True:
        if not cursos:
            print("No hay cursos registrados.")
            ...
            # Define los índices inicial y final para la búsqueda binaria.
            inicio, fin = 0, len(cursos) - 1
            encontrado = False
            while inicio <= fin:
                # Calcula el índice medio para dividir la búsqueda en la mitad.
                medio = (inicio + fin) // 2
...
# Define la funcion simular_cola
def simular_cola():
    while True:
        # Crea una lista vacía que actuará como cola para almacenar
        cola = []
...
# Define la funcion mostrar_historial
def mostrar_historial():
    while True:
        if not historial_cambios:
            ...
            # Recorre el historial en orden inverso (más reciente primero) enumerando los cambios.
